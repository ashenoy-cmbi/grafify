...
1. easily make common types of grafs based on `ggplot2` but with fewer lines of code
...
knitr::include_graphics("all_graphsv0.2.0.jpg")
```
...
2. `simple_anova` generates type II ANOVA table through `car::Anova()`, which is now a dependency. v0.1.0 and v0.2.0 generated type I ANOVA table through `stats::anova()`. 
...
2. two new types of grafs are possible with two quantitative X-Y plots with a third variable that is either numeric (`plot_xy_NumGroup`) or categorical (`plot_xy_CatGroup`).
