...
`grafify` requires the following packages to be installed: `broom.mixed`, `magrittr`, `ggplot2`, `lme4`, `lmerTest`, `emmeans`, `car`, `pbkrtest`, `tidyr`, `purrr` and `Hmisc`.
 
### Motivation 

I made this package mainly to make it easier for plotting graphs during data exploration, teaching purposes to introduce linear models. I wanted to:
...
`grafify` has five main features (graphs, colour blind-friendly colour schemes, ANOVAs & post-hoc comparisons, practice datasets and data simulations) briefly described below:
...
1. There are 19 `plot_` functions of 6 broad types in `grafify`. The `plot_scatter..` versions are preferred when there are many data points, `plot_dot..` versions have a "cleaner" layout for smaller datasets. 
     ...
     **New since v1.5.0**: new `plot_` functions like the above but ending in `_sc` for *same colour* or *single colour* which plot boxes/dots/violins in one colour. The colour can be picked from `all_grafify` palettes or defined with a hexcode. 
