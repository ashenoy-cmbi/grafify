#' Mixed model summary from analyses of a model with random factors.
#'
#' This function uses \code{\link[lmerTest]{lmer}} to fit a linear mixed effect model and provides the full \code{\link{summary}}. It requires a data table, one dependent variable (Y_value), one or more independent variables (Fixed_Factor), and at least one random factor (Random_Factor).
#' This function is related to \code{\link{mixed_model}} and \code{\link{mixed_anova}}.
#'
...
#'
#' @return This function returns the output of from \code{summary(lmerTest::lmer)}.
#' @export mixed_mod_summary
#'
#' @examples
#' #Basic usage where the table Chol is passed with names of variables within quotes
#' mixed_mod_summary(Chol, "Cholesterol", c("Treatment", "Hospital"), "Subject")
#'
#' mixed_mod_summary(Tab_doublings, "Doubling_time", "Student", "Experiment")


mixed_mod_summary <- function(data, Y_value, Fixed_Factor, Random_Factor, ...){
